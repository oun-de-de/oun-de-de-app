...
import { AccountingNatureBadge } from '@/pages/dashboard/accounting/components/accounting-badges';
...
export const getColumnsForItem = (
  activeItem: string,
  canEdit = true,
): ColumnDef<SettingsRow>[] => {
  ...
  if (activeItem === 'Warehouse') {
    return [
      ...
      ...(canEdit ? [getEditActionColumn({ size: 40 })] : []),
    ];
  }
  ...
  if (activeItem === 'Unit') {
    return [
      ...
      }),
      {
        ...
        cell: ({ row }) => (
          <Badge variant={TYPE_BADGE_VARIANT[row.original.type] || 'default'}>
            {row.original.type}
          </Badge>
        ),
      },
      ...(canEdit
        ? [getEditActionColumn({ size: 40, buttonSize: 'icon' })]
        : []),
    ];
  }
  ...
  if (activeItem === 'Supplier') {
    return [
      ...
      ...(canEdit
        ? [getEditActionColumn({ size: 50, buttonSize: 'icon' })]
        : []),
    ];
  }
  ...
  if (activeItem === 'Account Type') {
    return [
      ...
      }),
      {
        ...
        meta: { bodyClassName: 'text-center' },
        cell: ({ row }) => (
          <AccountingNatureBadge value={row.original.type} />
        ),
      },
      getTruncateColumn({
        header: 'Description',
  ...
  return [
    ...
    ...(canEdit
      ? [getEditActionColumn({ header: '', headerClassName: 'w-12' })]
      : []),
  ];
};
