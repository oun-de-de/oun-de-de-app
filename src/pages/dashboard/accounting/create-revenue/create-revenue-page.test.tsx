...
import CreateRevenuePage from "./create-revenue-page";
...
vi.mock("../hooks/use-accounting-reference-data", () => ({
	useAccountingReferenceData: () => ({
		chartAccounts: [{ id: "acc-1", code: "4000", name: "Sales", accountType: { id: "type-1", code: "REV", name: "Revenue" } }],
...
describe("CreateRevenuePage", () => {
	...
	it("navigates back to accounting when Save & Close succeeds", async () => {
		const user = userEvent.setup();
		render(<CreateRevenuePage />);

		await user.clear(screen.getByLabelText("Ref No"));
		await user.type(screen.getByLabelText("Ref No"), "REV-0002");
		await user.clear(screen.getByLabelText("Date"));
		await user.type(screen.getByLabelText("Date"), "2025-04-22T11:00");
		...
		await user.type(screen.getByLabelText("Memo"), "Morning revenue");
		...
		await user.type(screen.getByLabelText("Line Memo"), "Shift sales");
		await user.clear(screen.getByLabelText("Line Amount"));
		await user.type(screen.getByLabelText("Line Amount"), "40");

		await user.click(screen.getByRole("button", { name: "Save & Close" }));

		await waitFor(() => {
			expect(mutateAsyncMock).toHaveBeenCalledTimes(1);
			expect(navigateMock).toHaveBeenCalledWith("/dashboard/accounting");
		});
	});

	it("resets the form when Save & New succeeds", async () => {
		const user = userEvent.setup();
		render(<CreateRevenuePage />);

		await user.clear(screen.getByLabelText("Ref No"));
		await user.type(screen.getByLabelText("Ref No"), "REV-0003");
		await user.clear(screen.getByLabelText("Date"));
		await user.type(screen.getByLabelText("Date"), "2025-04-22T12:00");
		await user.clear(screen.getByLabelText("Currency"));
		await user.type(screen.getByLabelText("Currency"), "currency-1");
		await user.clear(screen.getByLabelText("Employee"));
		await user.type(screen.getByLabelText("Employee"), "employee-1");
		await user.type(screen.getByLabelText("Memo"), "Lunch revenue");
		await user.clear(screen.getByLabelText("Line Account"));
		await user.type(screen.getByLabelText("Line Account"), "acc-1");
		await user.clear(screen.getByLabelText("Line Customer"));
		await user.type(screen.getByLabelText("Line Customer"), "customer-1");
		await user.type(screen.getByLabelText("Line Memo"), "Walk-in sales");
		await user.clear(screen.getByLabelText("Line Amount"));
		await user.type(screen.getByLabelText("Line Amount"), "60");
		...
		await waitFor(() => {
			...
			expect(screen.getByLabelText("Line Account")).toHaveValue("");
			expect(screen.getByLabelText("Line Customer")).toHaveValue("");
			expect(screen.getByLabelText("Line Memo")).toHaveValue("");
			expect(screen.getByLabelText("Line Amount")).toHaveValue(0);
		});

		expect(navigateMock).not.toHaveBeenCalledWith("/dashboard/accounting");
		expect((screen.getByLabelText("Ref No") as HTMLInputElement).value).toMatch(/^REV-\d+$/);
	});

	it("navigates to the revenue preview with printable state when Save & Print succeeds", async () => {
		const user = userEvent.setup();
		render(<CreateRevenuePage />);

		await user.clear(screen.getByLabelText("Ref No"));
		await user.type(screen.getByLabelText("Ref No"), "REV-0001");
		await user.clear(screen.getByLabelText("Date"));
		...
		await user.clear(screen.getByLabelText("Employee"));
		await user.type(screen.getByLabelText("Employee"), "employee-1");
		await user.type(screen.getByLabelText("Memo"), "Counter sales");
		await user.clear(screen.getByLabelText("Line Account"));
		await user.type(screen.getByLabelText("Line Account"), "acc-1");
		await user.clear(screen.getByLabelText("Line Customer"));
		await user.type(screen.getByLabelText("Line Customer"), "customer-1");
		await user.type(screen.getByLabelText("Line Memo"), "Retail revenue");
		await user.clear(screen.getByLabelText("Line Amount"));
		...
		await waitFor(() => {
			expect(navigateMock).toHaveBeenCalledWith("/dashboard/accounting/revenue-preview?paper=a5&orientation=landscape", {
				state: expect.objectContaining({
					returnPath: "/dashboard/accounting",
					receiptPaymentAmount: 25,
					receiptPaymentCode: "REV-0001",
