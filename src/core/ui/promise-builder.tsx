...
export function PromiseBuilder<T>({ promise, initialData, builder }: FutureBuilderProps<T>) {
	const [snapshot, setSnapshot] = useState<AsyncSnapshot<T>>(
		initialData !== undefined ? { connectionState: "none", data: initialData } : { connectionState: "none" },
	);
